    ...
    if (document.querySelector('.pagination-box') !== null) {
        ...
        pageNumberBox.forEach(element => {
            if (urlSearchParams.get('text') !== null) {
                var href = '<?php echo $_SERVER["PHP_SELF"]; ?>?page='+element+'&payVendor=<?php echo $_GET["payVendor"]; ?>&payStatus=<?php echo $_GET["payStatus"]; ?>&fromDate=<?php echo $_GET["fromDate"]; ?>&toDate=<?php echo $_GET["toDate"]; ?>&orderBy=<?php echo $_GET["orderBy"]; ?>&text='+urlSearchParams.get('text')+'';
            } else {
                var href = '<?php echo $_SERVER["PHP_SELF"]; ?>?page='+element+'&payVendor=<?php echo $_GET["payVendor"]; ?>&payStatus=<?php echo $_GET["payStatus"]; ?>&fromDate=<?php echo $_GET["fromDate"]; ?>&toDate=<?php echo $_GET["toDate"]; ?>&orderBy=<?php echo $_GET["orderBy"]; ?>';
            }
