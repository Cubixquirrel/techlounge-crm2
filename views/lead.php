...
if ((in_array('Admin', explode(',', $row_select_user['user_role']))) OR (in_array('Sales', explode(',', $row_select_user['user_role'])))) {
    $page_title = 'Lead';
    if (!isset($_GET['page']) ) {
        header ('location: ../views/lead.php?page=1&business=ALL&website=ALL&status=ALL&payVendor=ALL&amount=ALL&sales=ALL&processor=ALL&fromDate=01-01-2020&toDate='.$today.'&orderBy=Latest');
    }
} else {
...
<body>
    ...
    <script>
        document.querySelector('#lead-data-menu').setAttribute('class', 'sidebar-list active');
    </script>
    ...
    include_once('../components-table/lead-data.php');
    ?>
    <script>
        document.querySelector('.info-box:nth-child(1)').setAttribute('class', 'info-box active');
    </script>
