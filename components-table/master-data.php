...
$sql_select_forms = 'SELECT * FROM forms WHERE id != "" ' . $sql_text . $business . $website . $status . $payVendor . $amount . $sales . $processor . $betweenDate . '';
...
$sql_select_forms = 'SELECT * FROM forms WHERE id != "" ' . $sql_text . $business . $website . $status . $payVendor . $amount . $sales . $processor . $betweenDate . ' ORDER BY STR_TO_DATE(date, "%d-%m-%Y %T") '.$orderBy.' LIMIT '.$page_first_result.','.$results_per_page.'';
$result_select_forms = $conn->query($sql_select_forms);

if (isset($_GET['export']) && $_GET['export'] == true) {
    $sql_select_forms_export = 'SELECT * FROM forms WHERE id != "" ' . $sql_text . $business . $website . $status . $payVendor . $amount . $sales . $processor . $betweenDate . ' ORDER BY STR_TO_DATE(date, "%d-%m-%Y %T") '.$orderBy.'';
    $exp_table = 'dashboard';
...
<div class="action-main-box">
    <select id="mark-to-sales" onchange="markData('mark-to-sales', 'Sales')">
        ...
        <?php
        $sql_select_users = 'SELECT * FROM users WHERE FIND_IN_SET("Sales", user_role)';
    ...
    <select id="mark-to-processor" onchange="markData('mark-to-processor', 'Processor')">
        ...
        <option disabled>--------------</option>
        <?php
        $sql_select_users = 'SELECT * FROM users WHERE FIND_IN_SET("Processor", user_role)';
        $result_select_users = $conn->query($sql_select_users);
        if ($result_select_users->num_rows > 0) {
            while ($row_select_users = $result_select_users->fetch_assoc()) {
                echo '<option value="'.$row_select_users["user_name"].'">'.$row_select_users["user_name"].'</option>';
...
<?php
if (($result_select_forms->num_rows > 0) && ($display == 'on')) {
