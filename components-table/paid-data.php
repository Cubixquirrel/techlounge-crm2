...
if (in_array('Admin', explode(',', $row_select_user['user_role']))) {
    $sql_select_forms = 'SELECT * FROM forms WHERE id != "" ' . $sql_text . $business . $website . $status . $payVendor . $amount . $sales . $processor . $betweenDate . $stage . ' AND status = "Paid"';
} else {
    $sql_select_forms = 'SELECT * FROM forms WHERE id != "" ' . $sql_text . $business . $website . $status . $payVendor . $amount . $sales . $processor . $betweenDate . $stage . ' AND status = "Paid" AND pay_vendor = "kfs.jamshedpur@gmail.com" AND assigned_to = "'.$user_name.'"';
}
...
if (in_array('Admin', explode(',', $row_select_user['user_role']))) {
    $sql_select_forms = 'SELECT * FROM forms WHERE id != "" ' . $sql_text . $business . $website . $status . $payVendor . $amount . $sales . $processor . $betweenDate . $stage . ' AND status = "Paid" ORDER BY STR_TO_DATE(date, "%d-%m-%Y %T") '.$orderBy.' LIMIT '.$page_first_result.','.$results_per_page.'';
} else {
    $sql_select_forms = 'SELECT * FROM forms WHERE id != "" ' . $sql_text . $business . $website . $status . $payVendor . $amount . $sales . $processor . $betweenDate . $stage . ' AND status = "Paid" AND pay_vendor = "kfs.jamshedpur@gmail.com" AND assigned_to = "'.$user_name.'" ORDER BY STR_TO_DATE(date, "%d-%m-%Y %T") '.$orderBy.' LIMIT '.$page_first_result.','.$results_per_page.'';
}
$result_select_forms = $conn->query($sql_select_forms);

if (isset($_GET['export']) && $_GET['export'] == true) {
    if (in_array('Admin', explode(',', $row_select_user['user_role']))) {
        $sql_select_forms_export = 'SELECT * FROM forms WHERE id != "" ' . $sql_text . $business . $website . $status . $payVendor . $amount . $sales . $processor . $betweenDate . $stage . ' AND status = "Paid" ORDER BY STR_TO_DATE(date, "%d-%m-%Y %T") '.$orderBy.'';
    } else {
        $sql_select_forms_export = 'SELECT * FROM forms WHERE id != "" ' . $sql_text . $business . $website . $status . $payVendor . $amount . $sales . $processor . $betweenDate . $stage . ' AND status = "Paid" AND pay_vendor = "kfs.jamshedpur@gmail.com" AND assigned_to = "'.$user_name.'" ORDER BY STR_TO_DATE(date, "%d-%m-%Y %T") '.$orderBy.'';
    }
    $exp_table = 'paid';
        ...
        <div class="table flex-table" table-data-id="<?php echo $row_select_forms['id']; ?>" onclick="selectTableDataId('<?php echo $row_select_forms['id']; ?>')">
            ...
            <div class="table-right">
                <div class="third-inner">
                    ...
                    <?php
                    if (($row_select_forms['dl_application_number'] != '') OR ($row_select_forms['date_slot'] != '')) {
                        if ($row_select_forms['dl_application_number'] != '') {
                            $dl_application_number = $row_select_forms['dl_application_number'];
                        } else {
                            $dl_application_number = '-';
                        }

                        if ($row_select_forms['date_slot'] != '') {
                            $date_slot = $row_select_forms['date_slot'];
                        } else {
                            $date_slot = '-';
                        }
                        ?>
                        <div class="dl-inner">
                            <span class="dl-application-number-label">Application Number: <?php echo $dl_application_number; ?></span>
                            <span class="dl-application-number-label">Date Slot: <?php echo $date_slot; ?></span>
                        </div>
                        <?php
                    }
                    ?>
