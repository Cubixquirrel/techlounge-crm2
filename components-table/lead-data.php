...
if (in_array('Admin', explode(',', $row_select_user['user_role']))) {
    $sql_select_forms = 'SELECT * FROM forms WHERE id != "" ' . $sql_text . $business . $website . $status . $payVendor . $amount . $sales . $processor . $betweenDate . ' AND dropped != "true" AND status != "Paid" AND is_follow_up != "true"';
} else {
    $sql_select_forms = 'SELECT * FROM forms WHERE id != "" ' . $sql_text . $business . $website . $status . $payVendor . $amount . $sales . $processor . $betweenDate . ' AND dropped != "true" AND status != "Paid" AND is_follow_up != "true" AND assigned_to = "'.$user_name.'"';
}
...
if (in_array('Admin', explode(',', $row_select_user['user_role']))) {
    $sql_select_forms = 'SELECT * FROM forms WHERE id != "" ' . $sql_text . $business . $website . $status . $payVendor . $amount . $sales . $processor . $betweenDate . ' AND dropped != "true" AND status != "Paid" AND is_follow_up != "true" ORDER BY STR_TO_DATE(date, "%d-%m-%Y %T") '.$orderBy.' LIMIT '.$page_first_result.','.$results_per_page.'';
} else {
    $sql_select_forms = 'SELECT * FROM forms WHERE id != "" ' . $sql_text . $business . $website . $status . $payVendor . $amount . $sales . $processor . $betweenDate . ' AND dropped != "true" AND status != "Paid" AND is_follow_up != "true" AND assigned_to = "'.$user_name.'" ORDER BY STR_TO_DATE(date, "%d-%m-%Y %T") '.$orderBy.' LIMIT '.$page_first_result.','.$results_per_page.'';
}
$result_select_forms = $conn->query($sql_select_forms);

if (isset($_GET['export']) && $_GET['export'] == true) {
    if (in_array('Admin', explode(',', $row_select_user['user_role']))) {
        $sql_select_forms_export = 'SELECT * FROM forms WHERE id != "" ' . $sql_text . $business . $website . $status . $payVendor . $amount . $sales . $processor . $betweenDate . ' AND dropped != "true" AND status != "Paid" AND is_follow_up != "true" ORDER BY STR_TO_DATE(date, "%d-%m-%Y %T") '.$orderBy.'';
    } else {
        $sql_select_forms_export = 'SELECT * FROM forms WHERE id != "" ' . $sql_text . $business . $website . $status . $payVendor . $amount . $sales . $processor . $betweenDate . ' AND dropped != "true" AND status != "Paid" AND is_follow_up != "true" AND assigned_to = "'.$user_name.'" ORDER BY STR_TO_DATE(date, "%d-%m-%Y %T") '.$orderBy.'';
    }
    $exp_table = 'lead';
...
<div class="action-main-box">
    ...
    <span id="dropped" onclick="markDrop('drop-modal')">Drop</span>
</div>

<?php
if (($result_select_forms->num_rows > 0)) {
    while ($row_select_forms = $result_select_forms->fetch_assoc()) {
        $mobile = intval($row_select_forms['mobile']);
            ...
            <div class="table-left">
                ...
                <div class="table-second">
                    ...
                    if ($row_select_forms['processor'] == '') {
                        ...
                        <div class="second-inner">                        
                            <span>Location: <span class="location-column"><?php echo $location; ?><span class="<?php echo $signal; ?>"></span></span>
                        </div>
                ...
                <div class="table-third">
                    <div class="third-inner">
                        ...
                        <div class="action-column lead-action-column">
                            <span class="lead-action interested" id="interested-<?php echo $row_select_forms['id']; ?>" onclick="updateInterestedTimeline('<?php echo $row_select_forms['id']; ?>')">Interested</span>

                            <select class="lead-action not-interested" id="not-interested-<?php echo $row_select_forms['id']; ?>" onchange="updateNotInterestedTimeline('<?php echo $row_select_forms['id']; ?>')">
                                <option>Not Interested</option>
                                <option value="" disabled>--------------</option>
                                <option value="Costly">Costly</option>
                                <option value="Free at Govt. Site">Free at Govt. Site</option>
                                <option value="Already Done">Already Done</option>
                            </select>

                            <a class="lead-action call" href="tel:<?php echo $mobile; ?>"><i class="icon-phone1"></i></a>
                            <a class="lead-action sms" href="../views/send.php?clientId=<?php echo $row_select_forms['id']; ?>&type=sms"><i class="icon-send1"></i></a>
                            <a class="lead-action email" href="../views/send.php?clientId=<?php echo $row_select_forms['id']; ?>&type=email"><i class="icon-mail"></i></a>
                            <a class="lead-action whatsapp" href="../views/send.php?clientId=<?php echo $row_select_forms['id']; ?>&type=whatsapp"><i class="icon-message-circle"></i></a>
                        </div>
                        
                        <span class="not-interested-message" id="not-interested-message-<?php echo $row_select_forms['id']; ?>"></span>
                    </div>
                </div>
            </div>
